...
def saveEnquery(request):
    if request.method == "POST":
        ...
        # Check if a Service object with the same box_number exists
        existing_service = Service.objects.filter(box_number=box_no).first()

        if existing_service:
            # Update the existing Service object with new data
            existing_service.medicine_name = name
            existing_service.quantity = quantity
            existing_service.save()
        else:
            # Create a new Service object
            new_service = Service(box_number=box_no, medicine_name=name, quantity=quantity)
            new_service.save()
...
def check_reminders(request):
    ...
    # If there are reminders to show, redirect to show_reminders
    if reminders_to_show:
        return redirect('show_reminders')  # Redirect to 'show_reminders' URL
    else:
        return render(request, 'index.html')  # Render another page if no reminders
